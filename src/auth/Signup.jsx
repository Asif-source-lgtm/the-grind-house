...
const Signup = () => {
  ...
  return (
    <div className="min-h-screen bg-bg-dark flex flex-col justify-center items-center p-4 relative overflow-hidden font-sans">
      <div className="absolute top-1/4 right-1/4 w-96 h-96 bg-brand-purple/10 blur-[120px] rounded-full point-events-none"></div>
      <div className="absolute bottom-1/4 left-1/4 w-96 h-96 bg-brand-cyan/10 blur-[120px] rounded-full point-events-none"></div>
      ...
      <div className="glass w-full max-w-md p-8 md:p-10 rounded-2xl border border-white/10 shadow-2xl relative z-10">
        <div className="text-center mb-8">
          <h2 className="text-3xl font-black text-white mb-2 tracking-wide">JOIN THE GRIND</h2>
          <p className="text-text-secondary text-sm">Create an account to begin your journey.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-5">
          ...
          <button 
            type="submit" 
            disabled={submitting}
            className="w-full mt-2 bg-gradient-to-r from-brand-cyan to-brand-purple hover:opacity-90 text-white font-bold py-3.5 rounded-xl shadow-lg shadow-brand-purple/20 transition-all flex items-center justify-center gap-2 group disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {submitting ? 'Creating Account...' : <>Create Account <ArrowRight size={18} className="group-hover:translate-x-1 transition-transform" /></>}
          </button>
        </form>

        <p className="mt-8 text-center text-sm text-text-secondary">
          Already have an account? <Link to="/login" className="text-white font-bold hover:text-brand-purple transition-colors">Sign in</Link>
        </p>
      </div>
    </div>
  );
